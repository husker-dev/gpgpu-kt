...
abstract class SimpleCProgram(
    ...
    protected open fun stringifyFunctionCallExpression(
        ...
        val passContextVars = useLocalStruct && !isPredefined
        ...
        if(arrayIndex != null){
            stringifyExpression(header, buffer, arrayIndex)
            if (expression.arguments.isNotEmpty() || passContextVars)
                buffer.append(",")
        }
        if(passContextVars) {
        ...
        expression.arguments.forEachIndexed { i, arg ->
            ...
            if(isPredefined && needCast)
                buffer.append("(").append(toCType(header, function.argumentsTypes[i])).append(")(")
            stringifyExpression(header, buffer, arg)
            if(needCast)
                buffer.append(")")
